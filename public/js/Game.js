class Game {
    ...
    drawGrid() {
        ...
        this.ctx.beginPath();
        for (let x = startX; x < Math.min(endX, 3000); x += gridSize) {
            this.ctx.moveTo(x, Math.max(startY, 0));
            this.ctx.lineTo(x, Math.min(endY, 3000));
        }
        for (let y = startY; y < Math.min(endY, 3000); y += gridSize) {
            this.ctx.moveTo(Math.max(startX, 0), y);
